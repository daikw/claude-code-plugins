...
1. 実際のタスクでスキルを使う（テスト用プロンプトではなく本番に近いもの）
2. Claude がスキルを選択しない場合 → description を見直す
3. Claude が間違ったことをする場合 → SKILL.md の当該セクションを強化
4. Haiku でも動くか確認（Opus 向けに最適化すると Haiku では過少情報になる）
